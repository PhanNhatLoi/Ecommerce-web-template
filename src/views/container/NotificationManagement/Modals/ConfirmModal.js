...
const ConfirmModal = (props) => {
    ...
    for (const id of confirmIds) {
      action(id)
        .then((res) => {
          ...
          AMessage.success(t(`${actionType}_notification_success`));
          setLoading(false);
          handleCancel();
        })
        .catch((err) => {
  ...
  return (
    <AntModal
      title={t(`${actionType}_notification`)}
      description={t(`${actionType}_notification_des`)}
      width={1000}
      ...
      <Form //
        ...
        <ATypography className="mb-3">{t(`${actionType}_notification_helper`)}</ATypography>
        <div className="mb-3">
        ...
        <MInputStyled //
          ...
          label=""
          name="confirmId"
          placeholder={t('enter_notification_id')}
          extra={<span style={{ fontSize: '11px' }}>{t('remove_notification_extra')}</span>}
        />
