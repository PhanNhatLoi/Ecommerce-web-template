...
const ConfirmModal = (props) => {
    ...
    for (const id of confirmIds) {
      action(id)
        .then((res) => {
          ...
          AMessage.success(t(`${actionType}_bank_success`));
          setLoading(false);
          handleCancel();
        })
        .catch((err) => {
  ...
  const onFinish = (values) => {
    confirmAction(values, props.deleteBankAccount);
  };
  ...
  return (
    <AntModal
      title={t(`${actionType}_bank`)}
      // description={t(`${actionType}_bank_des`)}
      width={1000}
      ...
      <Form //
        ...
        <ATypography className="mb-3">{t(`${actionType}_bank_banking`)}</ATypography>
        <div className="mb-3">
          {props.data.map((item, i) => (
            <ATypography strong>
              {item.id}{' '}
              <ATypography className="text-muted">
                ({item.name}){`${i < props.data.length - 1 ? ';' : ''}`}{' '}
              </ATypography>
            </ATypography>
          ))}
        </div>
        ...
        <MInputStyled //
          ...
          label=""
          name="confirmId"
          placeholder={t('enter_bank_id')}
          extra={<span style={{ fontSize: '11px' }}>{t('delete_bank_extra')}</span>}
        />
