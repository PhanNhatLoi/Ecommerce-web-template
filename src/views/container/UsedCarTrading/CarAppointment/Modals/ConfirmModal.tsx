...
type ConfirmModalProps = {
  ...
  modalShow: boolean;
  setModalShow: any;
  UpdateStatusCarAppointment: any;
};
...
const ConfirmModal: React.FC<ConfirmModalProps> = (props) => {
  ...
  const confirmAction = async (values: any, action: any) => {
    ...
    await Promise.all(
      ...
      .then((res) => {
        ...
        AMessage.success(t(`${actionType}_cartrading_appointment_success`));
        setLoading(false);
        handleCancel();
      })
      .catch((err) => {
  ...
  const onFinish = (values: any) => {
    confirmAction(values, props.UpdateStatusCarAppointment);
  };
  ...
  return (
    <AntModal
      title={t(`${actionType}_cartrading_appointment`)}
      description={t(`${actionType}_cartrading_appointment_des`)}
      width={1000}
      ...
      <Form //
        ...
        <ATypography className="mb-3">{t(`${actionType}_cartrading_appointment_helper`)}</ATypography>
        <div className="mb-3">
        ...
        <MInputStyled //
          ...
          label=""
          name="confirmId"
          placeholder={t('enter_cartrading_appointment_id')}
          extra={<span style={{ fontSize: '11px' }}>{t(`cartrading_appointment_extra`)}</span>}
        />
...
export default connect(null, {
  UpdateStatusCarAppointment: usedCarTradingAppointment.UpdateStatusCarAppointment
})(ConfirmModal);
