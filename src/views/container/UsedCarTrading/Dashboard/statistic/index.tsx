...
import { StatisticsResponse } from '~/state/ducks/carAccessories/dashboard/actions';
...
const DashboardStatistic: React.FC<DashboardStatisticProps> = (props) => {
  ...
  useEffect(() => {
    setLoading(true);
    props
      .getStatistics({ fromDate: props.params.fromDate, toDate: props.params.toDate })
      .then((res: { content: StatisticsResponse }) => {
        setStatisticData(res?.content);
        setLoading(false);
      })
      .catch((err) => {
        console.error('🚀chiendev ~ file: index.tsx:29 ~ useEffect ~ err:', err);
        setLoading(false);
      });
  }, [props.params]);
  ...
  const statisticList = [
    ...
    {
      subTitle: t('average_order_value'),
      numbers: statisticData?.current?.averageOrderValue || 0,
      percent: calculateGrowthPercent(statisticData?.previous?.averageOrderValue || 0, statisticData?.current?.averageOrderValue || 0),
      hasPrevious: true,
      isPrice: true,
      prevStatistic: [
        {
          label: t('prev_period'),
          value: statisticData?.previous?.averageOrderValue || 0,
          color: 'color-grey'
        }
      ]
    },
    {
      subTitle: t('gross_profit'),
      numbers: statisticData?.current?.totalGrossProfit || 0,
      percent: calculateGrowthPercent(statisticData?.previous?.totalGrossProfit || 0, statisticData?.current?.totalGrossProfit || 0),
      hasPrevious: true,
      isPrice: true,
      prevStatistic: [
        {
          label: t('prev_period'),
          value: statisticData?.previous?.totalGrossProfit || 0,
          color: 'color-grey'
        }
      ]
    },
    {
      subTitle: t('sold_products'),
      numbers: statisticData?.current?.soleProducts || 0,
      percent: calculateGrowthPercent(statisticData?.previous?.soleProducts || 0, statisticData?.current?.soleProducts || 0),
      hasPrevious: true,
      prevStatistic: [
        {
          label: t('prev_period'),
          value: statisticData?.previous?.soleProducts || 0,
          color: 'color-grey'
        }
      ]
    },
    {
      subTitle: t('return_products'),
      numbers: statisticData?.current?.returnProducts || 0,
      percent: calculateGrowthPercent(statisticData?.previous?.returnProducts || 0, statisticData?.current?.returnProducts || 0),
      hasPrevious: true,
      prevStatistic: [
        {
          label: t('prev_period'),
          value: statisticData?.previous?.returnProducts || 0,
          color: 'color-grey'
        }
      ]
    }
