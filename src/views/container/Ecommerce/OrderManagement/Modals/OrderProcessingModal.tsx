...
const ConfirmModal: React.FC<ConfirmModalProps> = (props) => {
  ...
  const confirmAction = (body, action) => {
    action(body)
      .then((res) => {
        props.setNeedLoadNewData(true);
        AMessage.success(t(`${actionType}_order_success`));
        handleCancel();
        setLoading(false);
      })
      .catch((err) => {
  ...
  const onFinish = (values) => {
    setLoading(true);
    const body = { id: values.confirmId, status: actionType, note: '' };
    confirmAction(body, props.updateStatusSaleOrder);
  };
  ...
  return (
    <AntModal
      title={t(`${actionType}_order_process`)}
      // description={t(`${actionType}_order_process_desc`)}
      width={1000}
      ...
      <Form //
        ...
        <MInputStyled //
          ...
          extra={<span style={{ fontSize: '11px' }}>{t('remove_order_extra')}</span>}
        />
