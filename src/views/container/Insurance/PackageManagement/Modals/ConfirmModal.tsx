...
type ConfirmModalProps = {
  ...
  blockPackage: (id: string) => void;
  unBlockPackage: (id: string) => void;
};
...
const ConfirmModal: React.FC<ConfirmModalProps> = (props) => {
  ...
  const confirmAction = (values: { confirmId: string }, action: (id: string) => void) => {
    ...
    Promise.all(
      ...
      .then(() => {
        ...
        AMessage.success(t(`${actionType}_insurance_package_success`));
        setLoading(false);
        handleCancel();
      })
      .catch((err) => {
  ...
  const onFinish = (values: { confirmId: string }) => {
    switch (actionType) {
      case ACTION_TYPE.DELETE:
        confirmAction(values, props.deletePackage);
        break;
      case ACTION_TYPE.BLOCK:
        confirmAction(values, props.blockPackage);
        break;
      case ACTION_TYPE.UNBLOCK:
        confirmAction(values, props.unBlockPackage);
        break;
      default:
        break;
    }
  ...
  return (
    <AntModal
      title={t(`${actionType}_insurance_package`)}
      description={t(`${actionType}_insurance_package_des`)}
      width={1000}
      ...
      <Form //
        ...
        <ATypography className="mb-3">{t(`${actionType}_insurance_package_helper`)}</ATypography>
        <div className="mb-3">
        ...
        <MInputStyled //
          ...
          label=""
          name="confirmId"
          placeholder={t('enter_insurance_package_id')}
          extra={<span style={{ fontSize: '11px' }}>{t('remove_insurance_package_extra')}</span>}
        />
...
export default connect(null, {
  deletePackage: packageActions.deletePackage,
  blockPackage: packageActions.blockPackage,
  unBlockPackage: packageActions.unBlockPackage
})(ConfirmModal);
