...
const ConfirmModal: React.FC<ConfirmModalProps> = (props) => {
  ...
  const confirmAction = async (values: any, action: any) => {
    ...
    await Promise.all(
      ...
      .then((res) => {
        ...
        AMessage.success(t(`${actionType}_servicePromotion_success`));
        setLoading(false);
        handleCancel();
      })
      .catch((err) => {
  ...
  const onFinish = (values: any) => {
    switch (actionType) {
      case 'delete':
        confirmAction(values, props.deleteServicePromotion);
        break;
      default:
        break;
    }
  ...
  return (
    <AntModal
      title={t(`${actionType}ServicePromotion`)}
      description={t(`${actionType}ServicePromotionDes`)}
      width={1000}
      ...
      <Form //
        ...
        <ATypography className="mb-3">{t(`${actionType}ServicePromotionHelper`)}</ATypography>
        <div className="mb-3">
        ...
        <MInputStyled //
          ...
          label=""
          name="confirmId"
          placeholder={t('enterServicePromotionId')}
          extra={<span style={{ fontSize: '11px' }}>{t('removeServicePromotionExtra')}</span>}
        />
...
export default connect(null, {
  deleteServicePromotion: servicePromotionActions.deleteServicePromotion
})(ConfirmModal);
