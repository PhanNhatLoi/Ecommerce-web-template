...
const ConfirmModal = (props) => {
    ...
    for (const id of confirmIds) {
      action(id)
        .then((res) => {
          ...
          AMessage.success(t(`${actionType}_quotation_success`));
          setLoading(false);
          handleCancel();
        })
        .catch((err) => {
  ...
  const onFinish = (values) => {
    switch (actionType) {
      case 'cancel':
        confirmAction(values, props.cancelQuotation);
        break;
      case 'delete':
        confirmAction(values, props.cancelQuotation);
        break;
  ...
  return (
    <AntModal
      title={t(`${actionType}_quotation`)}
      description={t(`${actionType}_quotation_des`)}
      width={1000}
      ...
      <Form //
        ...
        <ATypography className="mb-3">{t(`${actionType}_quotation_helper`)}</ATypography>
        <div className="mb-3">
        ...
        <MInputStyled //
          ...
          label=""
          name="confirmId"
          placeholder={t('enter_quotation_id')}
          extra={<span style={{ fontSize: '11px' }}>{t('remove_quotation_extra')}</span>}
        />
