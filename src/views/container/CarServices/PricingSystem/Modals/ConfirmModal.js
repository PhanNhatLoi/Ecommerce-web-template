import { CheckCircleOutlined, CloseCircleOutlined, CloseOutlined, DeleteOutlined } from '@ant-design/icons';
...
const ConfirmModal = (props) => {
  ...
  const confirmAction = async (confirmIds, action) => {
    setLoading(true);
    await Promise.all(
      confirmIds.map((id) => {
        return action(id);
      ...
      .then((res) => {
        ...
        AMessage.success(t(`${actionType}_pricing_product_success`));
        setLoading(false);
        handleCancel();
      })
      .catch((err) => {
  ...
  const onFinish = (values) => {
    ...
    switch (actionType) {
      case 'delete':
        confirmAction(confirmIds, props.deletePricingProduct);
        break;
      case 'deactivate':
        confirmAction(confirmIds, props.deactivatePricingProduct);
        break;
      case 'activate':
        confirmAction(confirmIds, props.activatePricingProduct);
        break;
      default:
        break;
    }
  ...
  return (
    <AntModal
      title={t(`${actionType}_pricing_product`)}
      ...
      <Form //
        ...
        <ATypography className="mb-3">{t(`${actionType}_pricing_product_helper`)}</ATypography>
        <div className="mb-3">
        ...
        <MInputStyled //
          ...
          label=""
          name="confirmId"
          placeholder={t('enter_pricing_product_id')}
          extra={<span style={{ fontSize: '11px' }}>{t('remove_pricing_product_extra')}</span>}
        />
...
export default connect(null, {
  deletePricingProduct: pricingSystemActions.deletePricingProduct,
  ...
  activatePricingProduct: pricingSystemActions.activatePricingProduct,
  deactivatePricingProduct: pricingSystemActions.deactivatePricingProduct
})(ConfirmModal);
