...
const QuotationInfoForm = (props) => {
  ...
  const onFinish = (values) => {
    ...
    if (values.items && values.items.length > 0) {
      if (values.items.some((value) => value.price < 0 || value.quantity <= 0)) return AMessage.error(t('update_quotation_failed'));
    }
    ...
    Promise.all(
      requestList.map((request) => {
        ...
        return props.updateQuotation(body);
      })
    )
      .then((res) => {
        AMessage.success(t('update_quotation_success'));
        props.onCancel();
        setSubmitting(false);
  ...
  const onFinishFailed = (err) => {
    console.error('🚀chiendev ~ file: QuotationInfoForm.js:165 ~ onFinishFailed ~ err:', err);
    setSubmitting(false);
  };
