...
const ConfirmModal = (props) => {
  ...
  const confirmAction = async (confirmIds, action) => {
    ...
    await Promise.all(
      ...
      .then((res) => {
        ...
        AMessage.success(t(`${actionType}_suppliers_success`));
        setLoading(false);
        handleCancel();
      })
      .catch((err) => {
  ...
  const onFinish = (values) => {
    ...
    switch (actionType) {
      case 'delete':
        setLoading(true);
        props
          .deleteSupplier({ codes: props.data.map((item) => item?.code) })
          .then((res) => {
            props.setNeedLoadNewData(true);
            AMessage.success(t('delete_supplier_success'));
            setLoading(false);
            handleCancel();
          })
          .catch((err) => {
        ...
        break;
      case 'block':
        confirmAction(confirmIds, props.deactivateSupplier);
        break;
      case 'unblock':
        confirmAction(confirmIds, props.activateSupplier);
        break;
      default:
        break;
    }
  ...
  return (
    <AntModal
      title={t(`${actionType}_suppliers`)}
      description={t(`${actionType}_suppliers_des`)}
      width={1000}
      ...
      <Form //
        ...
        <ATypography className="mb-3">{t(`${actionType}_suppliers_helper`)}</ATypography>
        <div className="mb-3">
          {props.data.map((item, i) => (
            <ATypography strong>
              {item.id} <ATypography className="text-muted">{`${i < props.data.length - 1 ? ';' : ''}`} </ATypography>
            </ATypography>
          ))}
        </div>
        ...
        <MInputStyled //
          ...
          label=""
          name="confirmId"
          placeholder={t('enter_suppliers_id')}
          extra={<span style={{ fontSize: '11px' }}>{t('remove_suppliers_extra')}</span>}
        />
...
export default connect(null, {
  ...
  deactivateSupplier: supplierActions.deactivateSupplier,
  activateSupplier: supplierActions.activateSupplier
})(ConfirmModal);
