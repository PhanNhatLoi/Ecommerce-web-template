...
const ConfirmModal = (props) => {
    ...
    for (const id of confirmIds) {
      action(id)
        .then((res) => {
          ...
          AMessage.success(t(`${actionType}_help_success`));
          setLoading(false);
          handleCancel();
        })
        .catch((err) => {
  ...
  return (
    <AntModal
      title={t(`${actionType}_member_help`)}
      description={t(`${actionType}_member_help_des`)}
      width={1000}
      ...
      <Form //
        ...
        <ATypography className="mb-3">{t(`${actionType}_member_help_helper`)}</ATypography>
        <div className="mb-3">
        ...
        <MInputStyled //
          ...
          label=""
          name="confirmId"
          placeholder={t('enter_member_help_id')}
          extra={<span style={{ fontSize: '11px' }}>{t('remove_member_help_extra')}</span>}
        />
