...
const ConfirmModal = (props) => {
  ...
  const confirmAction = (confirmIds, action) => {
    setLoading(true);
    Promise.all(confirmIds.map((id) => action(id)))
      .then((res) => {
        props.setNeedLoadNewData(true);
        AMessage.success(t(`${actionType}_member_success`));
        setLoading(false);
        handleCancel();
      })
      .catch((err) => {
  ...
  const onFinish = (values) => {
    ...
    switch (actionType) {
      case 'approve':
        confirmAction(confirmIds, props.approveMember);
        break;
      case 'block':
        confirmAction(confirmIds, props.blockMember);
        break;
      case 'unblock':
        confirmAction(confirmIds, props.approveMember);
        break;
      case 'reject':
        Promise.all(confirmIds.map((id) => props.rejectMember(id, { note: values.rejectNotes })))
          .then((res) => {
            props.setNeedLoadNewData(true);
            AMessage.success(t(`${actionType}_member_success`));
            setLoading(false);
          ...
          .catch((err) => {
            console.error('trandev ~ file: ConfirmModal.js ~ line 72 ~ onFinish ~ err', err);
            setLoading(false);
            AMessage.error(t(err.message));
          });
        break;
      case 'delete':
        confirmAction(confirmIds, props.deleteMember);
        break;
      default:
        break;
    }
  ...
  return (
    <AntModal
      title={t(`${actionType}_member`)}
      description={t(`${actionType}_member_des`)}
      width={1000}
      ...
      <Form //
        ...
        <ATypography className="mb-3">{t(`${actionType}_member_helper`)}</ATypography>
        <div className="mb-3">
        ...
        <MInputStyled //
          ...
          extra={<span style={{ fontSize: '11px' }}>{t('remove_member_extra')}</span>}
        />
...
export default connect(null, {
  ...
  blockMember: memberActions.blockMember,
