import React, { useEffect, useState } from 'react';
import { Image } from 'antd/es';
import { getAuthorizedUser } from '~/state/utils/session';
import { toAbsoluteUrl } from '~/views/utilities/helpers/AssetsHelpers';
import { DEFAULT_AVATAR } from '~/configs/default';
import { LoadingOutlined } from '@ant-design/icons';
import { firstImage, firstImageWithoutToken } from '~/views/utilities/helpers/utilObject';
import styled, { css } from 'styled-components';

const wrapStyle = css`
  padding: 4px;
  margin: 0px 6px;
  border: 1px solid #ebebeb;
  width: fit-content;
`;

const AuthImageStyled = styled.span`
  ${({ notHaveBorder }) => (notHaveBorder ? '' : wrapStyle)};
`;

function AuthImage(props) {
  const [imgSrc, setImgSrc] = useState('');
  const [error, setError] = useState(false);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (props.isAuth && props.src !== '') {
      setLoading(true);
      fetch(props.src, {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${getAuthorizedUser()}`,
          lang: 'en' // not care
        }
      })
        .then((res) => {
          if (res && res.status === 200)
            res.blob().then((blob) => {
              setImgSrc(URL.createObjectURL(blob));
              setError(false);
              setTimeout(() => {
                setLoading(false);
              }, 150);
            });
          else {
            setError(true);
            setLoading(false);
          }
        })
        .catch((err) => {
          console.error(`ithoangtan -  ~ file: AuthImage.js ~ line 60 ~ useEffect ~ err`, err);
          setError(true);
          setLoading(false);
        });
    } else setLoading(false);
  }, [props.src]);

  return (
    <AuthImageStyled
      notHaveBorder={props.notHaveBorder}
      className={props.containerClassName || 'd-flex align-items-center justify-content-center'}>
      {!loading && imgSrc !== '' && !error ? (
        <Image
          {...props}
          style={{ ...props.style, width: props.width, height: props.height }} // auto responsive
          alt={'ecaraid image'}
          // placeholder={<Image preview={false} src={toAbsoluteUrl(DEFAULT_AVATAR)} />}
          src={
            imgSrc !== ''
              ? imgSrc
              : props.src
              ? props.isAuth
                ? firstImage(props.src)
                : firstImageWithoutToken(props.src)
              : toAbsoluteUrl(DEFAULT_AVATAR)
          }
        />
      ) : (
        <>
          {loading ? (
            <LoadingOutlined />
          ) : (
            <Image
              style={{ width: props.width, height: props.height, maxWidth: '100%' }} // auto responsive
              src="error"
              fallback="data:image/png;base64,"
            />
          )}
        </>
      )}
    </AuthImageStyled>
  );
}
export default React.memo(AuthImage);
