...
function MUploadImageCrop(props) {
  ...
  useEffect(() => {
    if (Boolean(props.file)) setFile({ url: firstImage(props.file) });
    else setFile({ url: '' });
  ...
  const uploadButton = (
    <div>
      ...
      <div className="ant-upload-text px-2"> {props.uploadText || t('upload')}</div>
    </div>
  );
  ...
  return (
    <div
      ...
      <Form.Item label={props?.label} name={props.name || 'MUploadImageCrop'} rules={rules} extra={props.extra} {...props}>
        <ImgCrop
          // cropperProps={{}}
